"""Завдання буде схоже на попереднє, але тепер у тексті ми шукатимемо номер телефону
 України в міжнародному форматі, шаблон якого наступний: +380(67)777-7-777 або
  +380(67)777-77-77

Напишіть регулярний вираз для функції find_all_phones, яка буде в тексті
 (параметр text) знаходити всі телефонні номери за вказаним шаблоном та
...
def find_all_phones(text):
    result1 = re.findall(
        r"[+]380[(][0-9]{2}[)][0-9]{3}[-][0-9]{1,2}[-][0-9]{2,3}\b", text)
    result = []
    for i in result1:
        result.append(i[0:17])
    return result
